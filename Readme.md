...
В `worker.py` установлен `prefetch_count=1`, что гарантирует равномерное распределение задач между воркерами. Каждый воркер будет обрабатывать только одну задачу одновременно.
